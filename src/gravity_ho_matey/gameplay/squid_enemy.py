...
class SquidEnemy:
    ...
    def update_tentacles(self, dt: float, ship_pos: Vec2, ship_radius: float) -> None:
        ...
        for i in range(SQUID_TENTACLE_COUNT):
            ...
            if clinging or coiling:
                target = ship_pos + Vec2.from_angle(spread) * (ship_radius + 5.0)
                spring = 62.0
                drag = 0.78
                max_len = self.tentacle_reach * 1.05
            else:
                sway = math.sin(self.tentacle_wobble * 5.5 + i * 0.85) * 16.0
                lateral = math.cos(self.tentacle_wobble * 4.0 + i * 1.1) * 10.0
                dir_v = Vec2.from_angle(spread)
                target = self.pos + dir_v * self.tentacle_reach + Vec2(sway, lateral)
                spring = 26.0
                drag = 0.88
                max_len = self.tentacle_reach * 1.22
    ...
    def integrate(
        ...
        *,
        ...
        self.update_tentacles(dt, ship_pos, ship_radius)
        self.clinging = self.is_clinging(ship_pos, ship_radius)

        surface_gap = dist - self.orbit_radius

        if self.clinging:
            latch_dist = ship_radius + self.radius * 0.52
            desired = ship_pos - radial * latch_dist
            latch_blend = min(1.0, dt * 12.0)
            self.pos = Vec2(
                self.pos.x + (desired.x - self.pos.x) * latch_blend,
                self.pos.y + (desired.y - self.pos.y) * latch_blend,
            )
            self.vel = ship_vel * 0.96 + tangent * (34.0 * self.orbit_sign)
        ...
        if dist > self.engage_range:
            ...
            target_orbit = tangent * self.orbit_speed + ship_vel * 0.42
